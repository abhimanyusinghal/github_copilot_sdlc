...
- **Copilot Chat** — in VS Code, or the immersive chat at **github.com/copilot**.
...
1. Open **Copilot Chat** (VS Code, or **github.com/copilot**).
2. Open the **messy notes** for your track from `Lab_Materials/Module2_Lab/notes/` (e.g. `web-drupal-notes.md`).
3. Open your sample repo's **Issues** tab on github.com.
4. Confirm you can reach the **"Product Requirements" Space** (or, if it isn't set up, have `glossary.md`, `personas.md` and `nfr-standards.md` open so you can reference them).

**What you should see:** Chat, your notes, the repo's Issues tab, and the Space (or its docs) all open.
...
1. Go to **github.com/copilot**. In the prompt box, ask Copilot to draft an issue, targeting your repo with `owner/repo`:
   > In `owner/repo`, create an issue for `<your story>`. Use the acceptance criteria we just wrote. Add labels: `auth`, `ux`.
2. Copilot drafts a **title, body, labels and metadata** (mapping to your repo's issue templates if it has them). **Review and edit every field** — you own the final wording — then click **Create**.
3. **Draft an issue from an image.** Start a new prompt, **attach** `mockup-reset-password.png`, and add: *"Create an issue from this wireframe for the password-reset screen."* Review and create.
...
4. Create them as **native sub-issues**: open the parent issue and click **Create sub-issue** at the bottom of the description (or, from the terminal, `gh issue create --parent <n>`). A parent can hold up to 100 sub-issues.
...
1. Re-run your Exercise 1 (structure) or Exercise 2 (story) prompt — this time **using the "Product Requirements" Space**, or by attaching `glossary.md`, `personas.md` and `nfr-standards.md` to the chat.
...
- **Can't create an issue with Copilot:** use **github.com/copilot** and target the repo with `owner/repo`; or paste the drafted issue body into a new issue manually.
